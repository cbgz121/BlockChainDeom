...
func AdminLogin(c *gin.Context) {
	...
	// 创建用户表格
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS admin (id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(25), password VARCHAR(50), name VARCHAR(25), phone VARCHAR(20), admin_id VARCHAR(30))")
	if err != nil {
		panic(err.Error())
	}
	appG := app.Gin{C: c}
	...
	if err := c.ShouldBind(reqBody); err != nil {
        ...
        // 在数据库中插入用户
//        _, err = db.Exec("INSERT INTO admin (username, password, name,phone,admin_id) VALUES (?, ?, ?,?,?)", reqBody.Username, reqBody.Password, "管理员",17644859968,"ac0e7d037817094e")
//        if err != nil {
...
//        }
	...
	// 在数据库中查找用户
	row := db.QueryRow("SELECT username, password, admin_id FROM admin WHERE username = ?", reqBody.Username)
