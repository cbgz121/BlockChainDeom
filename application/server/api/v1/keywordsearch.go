...
func KeyWordSearch(c *gin.Context) {
	...
	query := "SELECT * FROM uuu WHERE " + strings.Join(conditions, " OR ")
...
func getColumns(db *sql.DB) ([]string, error) {
	...
	rows, err := db.Query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?", "test", "uuu")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
