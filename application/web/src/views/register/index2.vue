<template>
    <div class="login-container">
        <el-form :model="formData" ref="registerForm" :rules="rules" class="register-form" auto-complete="on"
            label-position="left">
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                username: [{ required: true, message: '请输入用户名', trigger: 'submit' }, { min: 4, max: 20, message: '用户名长度应为4到20个字符', trigger: 'submit' }],
                email: [{ required: true, message: '请输入邮箱', trigger: 'submit' }, { type: 'email', message: '邮箱格式不正确', trigger: 'submit' }],
                password: [{ required: true, message: '请输入密码', trigger: 'submit' }, { min: 6, message: '密码长度不能少于6位', trigger: 'submit' }],
                confirmPassword: [{ required: true, message: '请输入密码', trigger: 'submit' }, { min: 6, message: '密码长度不能少于6位', trigger: 'submit' }],
                phone: [{ required: true, message: '请输入手机号', trigger: 'submit' }, { pattern: /^1[34578]\d{9}$/, message: '手机号格式不正确', trigger: 'submit' }],
            },
        };
    },
    watch: {
